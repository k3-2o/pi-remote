...
try {
  ...
  check(
    "HTTP health without key = 200 (health is public)",
    resp.status === 200,
  );
  ...
  const msg1 = await new Promise((r) =>
    ws1.once("message", (d) => r(JSON.parse(d.toString()))),
  );
  check(
    "WS without key = rejected",
    msg1.type === "error" && msg1.code === "AUTH_FAILED",
  );
  ws1.close();
  ...
  const msg2 = await new Promise((r) =>
    ws2.once("message", (d) => r(JSON.parse(d.toString()))),
  );
  check(
    "WS wrong key = rejected",
    msg2.type === "error" && msg2.code === "AUTH_FAILED",
  );
  ws2.close();
  ...
  console.log(
    "  - WS key-based auth: requires auth header support (known gap)",
  );
} catch (err) {
